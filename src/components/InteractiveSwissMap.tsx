...
const InteractiveSwissMap: React.FC<InteractiveSwissMapProps> = ({ 
  ...
  return (
    <Box>
      ...
      <MapContainer>
        <svg width="100%" height="100%" viewBox="0 0 800 600" style={{ transition: 'all 0.3s ease' }}>
          <path d="M 50 100 L 150 80 L 250 60 L 350 70 L 450 90 L 550 120 L 650 150 L 700 200 L 720 250 L 700 300 L 650 350 L 550 400 L 450 450 L 350 480 L 250 500 L 150 480 L 50 450 L 30 400 L 20 350 L 30 300 L 50 250 L 70 200 L 50 100 Z" fill="#f0f0f0" stroke="#cccccc" strokeWidth="2" />
          
          {cantons.map((canton) => (
            <g key={canton.code}>
              <path d={canton.pathData} fill={selectedCanton === canton.code ? '#1976d2' : getColorByCount(canton.professionalCount)} stroke="#ffffff" strokeWidth={selectedCanton === canton.code ? 3 : 2} style={{ cursor: 'pointer', transition: 'all 0.3s ease', filter: hoveredCanton?.code === canton.code ? 'drop-shadow(3px 3px 6px rgba(0,0,0,0.6))' : 'none' }} onClick={() => handleCantonClick(canton)} onMouseEnter={() => handleCantonHover(canton)} onMouseLeave={() => handleCantonHover(null)} />
              <text x={canton.coordinates.x} y={canton.coordinates.y + 8} textAnchor="middle" fontSize="11" fill="white" fontWeight="bold" style={{ pointerEvents: 'none', textShadow: '2px 2px 2px rgba(0,0,0,0.9)' }}>{canton.code.toUpperCase()}</text>
              <text x={canton.coordinates.x} y={canton.coordinates.y - 12} textAnchor="middle" fontSize="9" fill="#333" fontWeight="bold" style={{ pointerEvents: 'none' }}>{canton.professionalCount}</text>
            </g>
          ))}
        </svg>

        {hoveredCanton && (
          <Card sx={{ position: 'absolute', top: 20, right: 20, minWidth: 280, zIndex: 1000, boxShadow: 4 }}>
            <CardContent>
              <Typography variant="h6" gutterBottom>{hoveredCanton.name}</Typography>
              <Typography variant="body2" color="textSecondary" gutterBottom>Capital: {hoveredCanton.capital}</Typography>
              <Typography variant="body2" color="textSecondary" gutterBottom>Région: {hoveredCanton.region}</Typography>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                <Typography variant="body1" fontWeight="bold">{hoveredCanton.professionalCount} professionnels</Typography>
                <Chip label={getStatusText(hoveredCanton.professionalCount)} size="small" sx={{ backgroundColor: getColorByCount(hoveredCanton.professionalCount), color: 'white' }} />
              </Box>
            </CardContent>
          </Card>
        )}
          ...
          {(() => {
            ...
            return canton ? (
              <Card sx={{ backgroundColor: '#e3f2fd' }}>
                <CardContent>
                  <Typography variant="h6" gutterBottom>🗺️ Canton sélectionné: {canton.name}</Typography>
                  <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                    <Typography variant="body2"><strong>Capital:</strong> {canton.capital}</Typography>
                    <Typography variant="body2"><strong>Région:</strong> {canton.region}</Typography>
                    <Typography variant="body2"><strong>Professionnels:</strong> {canton.professionalCount}</Typography>
                    <Chip label={getStatusText(canton.professionalCount)} size="small" sx={{ backgroundColor: getColorByCount(canton.professionalCount), color: 'white' }} />
                  </Box>
                </CardContent>
              </Card>
            ) : null;
